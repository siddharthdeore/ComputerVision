...
void createTrackbars(){
	//create window foresir trackbars
...
void drawObject(int x, int y, Mat &frame){
	...
	//smooth Transistion of x and y using siddharth's kickass algo
	fx = fx * alpha + x * (1 - alpha);
	fy = fy * alpha + y * (1-alpha);
	...
	if (y - 20>0)
		line(frame, Point(x, y), Point(x, y - 20), Scalar(0, 255, 0), 1);
	else line(frame, Point(x, y), Point(x, 0), Scalar(0, 255, 0), 2);
	if (y + 20<FRAME_HEIGHT)
		line(frame, Point(x, y), Point(x, y + 20), Scalar(0, 255, 0), 1);
	else line(frame, Point(x, y), Point(x, FRAME_HEIGHT), Scalar(0, 255, 0), 1);
	if (x - 20>0)
		line(frame, Point(x, y), Point(x - 20, y), Scalar(0, 255, 0), 1);
	else line(frame, Point(x, y), Point(0, y), Scalar(0, 255, 0), 1);
	if (x + 20<FRAME_WIDTH)
		line(frame, Point(x, y), Point(x + 20, y), Scalar(0, 255, 0), 1);
	else line(frame, Point(x, y), Point(FRAME_WIDTH, y), Scalar(0, 255, 0), 1);

//	putText(frame, intToString(x) + "," + intToString(y), Point(x, y + 30), 1, 1, Scalar(0, 255, 0), 1); // this one prints xy near crosshair
	putText(frame, intToString(x) + "," + intToString(y), Point(10, 30), 1, 1, Scalar(0, 0, 0), 1); // this one prints xy near tracking object text
	cout << "X = " << x;
	cout << "\t Y = " << y << endl;
...
void trackFilteredObject(int &x, int &y, Mat threshold, Mat &cameraFeed){
	...
	if (hierarchy.size() > 0) {
		...
		if (numObjects<MAX_NUM_OBJECTS){
			...
			if (objectFound == true){
				putText(cameraFeed, "Tracking Object", Point(10, 10), 1, 1, Scalar(0, 0, 0), 1);
		...
		else putText(cameraFeed, "TOO MUCH NOISE! ADJUST FILTER", Point(0, 50), 1, 1, Scalar(0, 0, 255), 1);
	}
}
